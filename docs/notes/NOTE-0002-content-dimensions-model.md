...
## Implemented Admin UI Notes

- Значения dimension в просмотре игры отображаются иерархически: родительское значение идёт перед
  дочерними.
- Дочерние значения визуально выделяются отступом слева.
- Drag-and-drop сортировка ограничена sibling-группой: родительские значения можно двигать только
  среди родительских, дочерние — только внутри своего `parent_id`.
- Сортировка обновляет `sort_order` только у значений той группы, где изменился порядок.
